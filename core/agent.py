"""
EDR Windows Agent - Main Agent Class
"""
...
class EDRAgent:
    ...
    def _on_process_event(self, event_data: Dict[str, Any]):
        ...
        try:
            with self.buffer_lock:
                self.data_buffer['processes'].append({
                ...
                # Limit buffer size
                if len(self.data_buffer['processes']) > self.config.get('monitoring', 'batch_size', 50):
                    self.data_buffer['processes'] = self.data_buffer['processes'][-50:]
    ...
    def _on_file_event(self, event_data: Dict[str, Any]):
        ...
        try:
            with self.buffer_lock:
                ...
                # Limit buffer size
                if len(self.data_buffer['files']) > self.config.get('monitoring', 'batch_size', 50):
                    self.data_buffer['files'] = self.data_buffer['files'][-50:]
    ...
    def _on_network_event(self, event_data: Dict[str, Any]):
        ...
        try:
            with self.buffer_lock:
                self.data_buffer['networks'].append({
                    'timestamp': datetime.utcnow().isoformat(),
                    'hostname': self.system_info.get('hostname'),
                    **event_data
                })
                ...
                # Limit buffer size
                if len(self.data_buffer['networks']) > self.config.get('monitoring', 'batch_size', 50):
                    self.data_buffer['networks'] = self.data_buffer['networks'][-50:]
    ...
    def _handle_kill_process_command(self, params: Dict[str, Any]):
        ...
        try:
            ...
            if self.process_actions:
                success = self.process_actions.terminate_process(process_id, process_name)
                if success:
                    self.logger.info(f"✅ Process {process_name} ({process_id}) terminated")
                else:
    ...
    def _handle_block_network_command(self, params: Dict[str, Any]):
        ...
        try:
            ...
            # This would implement network blocking via Windows Firewall
            self.logger.info(f"🚫 Network block requested: {ip_address}:{port}")
    ...
    def _handle_quarantine_file_command(self, params: Dict[str, Any]):
        ...
        try:
            ...
            # This would implement file quarantine
            self.logger.info(f"🔒 File quarantine requested: {file_path}")
